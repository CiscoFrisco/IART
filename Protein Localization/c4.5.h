...
public:
	MatrixCls(string Data_File, int limit = -1)
	{
	...
	string testElement(int i, int j)
	{
		return test[i][j];
	}
...
public:
	...
	void TestTree(MatrixCls Data_Matrix);
};
...
void TreeCls::TestTree(MatrixCls Data_Matrix)
{
	...
	int Lines_Number = Data_Matrix.testSizeY();
	int i, j, k, l, m;
	...
	for (i = 0; i < Lines_Number; i++)
	{
		...
		{
			Attributes_Value.push_back(Data_Matrix.testElement(i, j));
		}

		string Temp_Score;
		Temp_Score = Temp_TestTree(Kinds, Attributes, Attributes_Value, Score_Range);

		unsigned short real = translate(Data_Matrix.testElement(i, Data_Matrix.SizeX() - 1));
		unsigned short pred = translate(Temp_Score);
	...
	cout << "Accuracy: " << ((float)correct / Lines_Number) * 100.0 << "%\n\n";

	showStats(correctPred, sumReal, sumPred, Lines_Number);
}
